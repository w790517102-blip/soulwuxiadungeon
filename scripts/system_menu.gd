...
const SPECIAL_BOOK_IDS := {
	"book_poem_a_int": true,
	"book_poem_b_luck": true,
	"book_essay_c_pen_up": true,
	"book_essay_d_pen_resist": true,
}
const WEAPON_RULES = {
	"liuyu": {
		"weapon_1": ["劍"],
...
func _apply_pending_item_use_with_sequence(target) -> void:
	if target == null or _pending_item_use_id == "":
		return
	var handled_special_book := await _play_book_reading_then_apply(
		_pending_item_use_id,
		_pending_item_effect,
		_pending_item_amount,
		target
	)
	if handled_special_book:
		return
	_apply_world_item(_pending_item_use_id, _pending_item_effect, _pending_item_amount, target)

func _play_book_reading_then_apply(item_id: String, effect: String, amount: int, target) -> bool:
	if not SPECIAL_BOOK_IDS.has(item_id):
		return false
	var lines := _get_special_book_dialog_lines(item_id, target)
	if lines.is_empty():
		return false
	var before_count := int(InventorySync.get_item_by_id(item_id).get("count", 0))
	await _show_book_dialog_sequence(lines)
	_apply_world_item(item_id, effect, amount, target)
	var after_count := int(InventorySync.get_item_by_id(item_id).get("count", 0))
	var applied_successfully := after_count < before_count
	if applied_successfully and GlobalState and GlobalState.has_method("set_flag"):
		GlobalState.set_flag(_book_read_flag_name(item_id), true)
	return true

func _show_book_dialog_sequence(lines: Array) -> void:
	var dialog_manager := get_node_or_null("/root/GameRoot/DialogManager")
	if dialog_manager and dialog_manager.has_method("show_dialog_sequence"):
		await dialog_manager.show_dialog_sequence(lines)
		return
	print("[BookRead] DialogManager not found, skip sequence.")

func _book_read_flag_name(item_id: String) -> String:
	return "book_read_%s" % item_id

func _get_special_book_dialog_lines(item_id: String, target) -> Array:
	var portrait_path := str(_get_actor_value(target, "portrait_path", LIU_YU_BOOK_PORTRAIT))
	if portrait_path == "":
