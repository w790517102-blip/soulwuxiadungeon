...
func _build_lines_for_stage(main_stage: int) -> Array:
	var met := GlobalState.get_flag("met_yuheng_store_keeper_e")
	...
	if event_market_choice_observe and event_yuheng_market_melody:
		if not met:
			return [
				{ "text": "「柴米油鹽，樣樣俱全唷!」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「客倌慢慢看，想要甚麼儘管說，別客氣。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「你問我最近有甚麼值得一提的異樣嗎?」", "speaker": 1, "portrait": "res://assets/sprites/empty.png" },
				{ "text": "「真要說的話，這陣子客人都乖得不像樣，講話輕聲細語的，也不閒話家常了」", "speaker": 1 },
				{ "text": "「琴聲一響，全鎮像被灑了催眠粉似的，連我自己都懶得計較利潤了……」", "speaker": 1 },
				{ "text": "「呵(打哈欠)…客倌，等你看好了在跟我說嘿…」", "speaker": 1 },
]
	if main_stage <= 2:
		if event_market_choice_observe and event_yuheng_market_melody and met:
			return [
				{ "text": "「想要甚麼儘管開口…」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「(哈欠)你想出多少價也請隨意~」", "speaker": 1, "portrait": portrait_path },
			]
		else:
			return [
				{ "text": "「柴米油鹽，樣樣俱全唷!」", "speaker": 1, "portrait": portrait_path },
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SPACE:
		...
		_mark_flag_after_close = not GlobalState.get_flag("met_yuheng_store_keeper_e")

func reset_dialog_state():
	get_node("/root/GameRoot/LiuYu").can_move = true
	is_talking = false
	if _mark_flag_after_close:
		GlobalState.set_flag("met_yuheng_store_keeper_e", true)
