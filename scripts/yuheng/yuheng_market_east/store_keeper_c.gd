...
func _build_lines_for_stage(main_stage: int) -> Array:
	var met := GlobalState.get_flag("met_yuheng_store_keeper_c")
	...
	if event_market_choice_observe and event_yuheng_market_melody:
		if not met:
			return [
				...
				{ "text": "「剛出鍋的白切，來兩片嚐嚐？」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「甚麼？你說這琴音？」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「哎呀，這幾個月客人說話都輕，像怕驚動這琴聲似的。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「但感覺大家守著這琴音，也守得悶了，話都憋得太久了」", "speaker": 1, "portrait":  portrait_path },
				{ "text": "「只怕這樣下去會適得其反...」", "speaker": 1, "portrait":  portrait_path },
				{ "text": "「好比肉要滷得透，也得開蓋散氣。一直悶著，腥味出不去。」", "speaker": 1, "portrait":  portrait_path },
				{ "text": "「琴聲按住心火是好事，但總得有個口子…」", "speaker": 1, "portrait":  portrait_path },
				{ "text": "「算了~吃飯事要緊，少俠要不要來點東波肉？江湖路長，多吃點熱食，\n別讓胃餓著。」", "speaker": 1, "portrait":  portrait_path },
	]
	if main_stage <= 2:
		if event_market_choice_observe and event_yuheng_market_melody and met:
			return [
				{ "text": "「少俠要不要來點東波肉？江湖路長，多吃點熱食\n，別讓胃餓著。」", "speaker": 1, "portrait":  portrait_path },
		]
		else:
			return [
				{ "text": "「剛出鍋的白切，來兩片嚐嚐？」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「要肥要瘦直說，別客氣。」", "speaker": 1, "portrait": portrait_path },
			]
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SPACE:
		...
		_mark_flag_after_close = not GlobalState.get_flag("met_yuheng_store_keeper_c")

func reset_dialog_state():
	get_node("/root/GameRoot/LiuYu").can_move = true
	is_talking = false
	if _mark_flag_after_close:
		GlobalState.set_flag("met_yuheng_store_keeper_c", true)
