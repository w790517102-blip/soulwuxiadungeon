...
func _build_lines_for_stage(main_stage: int) -> Array:
	var met := GlobalState.get_flag("met_monk_doctrine")
	...
	if event_market_choice_observe and event_yuheng_market_melody:
		if not met:
			return [
				{ "text": "「邪典似瘟疫，即便不讀它，與染者爭辯，亦可能沾染。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「慎言是藥，止語非終。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "(低頭思索)", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「原來關鍵在『少爭』而非『不言』——盡量不起衝突，\n卻也不能把話永遠憋著。」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「情緒久壓易反噬，反而給邪典可趁之機。」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "...", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「那為何這裡的人，情緒被琴聲壓制的如此透徹」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「卻不見任何邪典反噬的徵兆?」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「這玉衡鎮的迷實在是太多了。」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				{ "text": "劉語塵：「恐怕不是我這一介外人，一時半刻可以搞懂的。」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
			]
	if main_stage <= 2:
		if event_market_choice_observe and event_yuheng_market_melody and met:
			return [
				{ "text": "「慎言是藥。止語，非終。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "（記下了。）", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
			]
		else:
			return [
				{ "text": "「慎言是藥。止語，非終。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "劉語塵：「這位僧人似乎是要跟我警告甚麼…」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" }
			]
	# ❷ 主線中期：對琴音的觀感（循環）
	elif main_stage <= 6:
		return [
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SPACE:
		...
		_mark_flag_after_close = not GlobalState.get_flag("met_monk_doctrine")
func reset_dialog_state():
	get_node("/root/GameRoot/LiuYu").can_move = true
	is_talking = false
	if _mark_flag_after_close:
		GlobalState.set_flag("met_monk_doctrine", true)
