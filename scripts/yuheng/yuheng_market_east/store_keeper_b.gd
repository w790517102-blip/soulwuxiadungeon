...
func _build_lines_for_stage(main_stage: int) -> Array:
	var met := GlobalState.get_flag("met_yuheng_store_keeper_b")
	...
	if main_stage <= 2:
		if not met:
			return [
				...
				{ "text": "「少俠要不要看看今早剛宰的新豬，筋理清、沒有腥。」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「我這攤主打刀子口利，肉入口鮮。你要前腿還是五花？」", "speaker": 1, "portrait": portrait_path },
				{ "text": "「你說這琴聲？放著倒也不錯，以前的客人都常常來殺價\n現在都安靜地買完肉就走人」", "speaker": 1, "portrait":  portrait_path },
				{ "text": "「曲調美、人清淨，客人也有禮許多，何壞之有?」", "speaker": 1, "portrait":  portrait_path },
			]
		else:
			return [
				{ "text": "「希望以後的客人都不會跟我討價還價。」", "speaker": 1, "portrait": portrait_path },
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SPACE:
		...
		_mark_flag_after_close = not GlobalState.get_flag("met_yuheng_store_keeper_b")

func reset_dialog_state():
	get_node("/root/GameRoot/LiuYu").can_move = true
	is_talking = false
	if _mark_flag_after_close:
		GlobalState.set_flag("met_yuheng_store_keeper_b", true)
