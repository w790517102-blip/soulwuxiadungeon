...
func _build_lines_for_stage(main_stage: int) -> Array:
	var met := GlobalState.get_flag("met_yuheng_patient")
	# ❶ 主線前期：首輪完整版 / 其後走精簡循環
	if not met:
			return [
				{ "text": "（男子低著頭輕咳，卻立刻環顧四周，壓低聲音", "speaker": 1, "portrait": "res://assets/sprites/empty.png" },
  				{ "text": "「嗚……咳……不、不敢咳太大聲，怕被當成……不穩的人。」", "speaker": 1 },
				{ "text": "劉語塵：「咳嗽只是身體不適，怎麼會被這樣看待？」", "speaker": 2, "portrait": "res://assets/sprites/Liu_Yu/LiuYu_headshot.png" },
				
				{ "text": "「你是外地來的吧……你不知道，這鎮上……最怕的，不是病，是讓別人\n『感覺到你有病』……」", "speaker": 1 },
				{ "text": "（他抿緊嘴唇，搖了搖頭，轉過身去）", "speaker": 1 },
]
	if main_stage <= 2:
			return [
				{ "text": "（他抿緊嘴唇，搖了搖頭，轉過身去）", "speaker": 1 },
...
func _unhandled_input(event):
	...
	if event is InputEventKey and event.pressed and event.keycode == KEY_SPACE:
		...
		_mark_flag_after_close = not GlobalState.get_flag("met_yuheng_patient")

func reset_dialog_state():
	get_node("/root/GameRoot/LiuYu").can_move = true
	is_talking = false
	if _mark_flag_after_close:
		GlobalState.set_flag("met_yuheng_patient", true)
