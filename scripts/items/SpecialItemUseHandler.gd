...
func _build_book_reading_lamp_whisper(actor_key: String) -> Array:
	match actor_key:
		"lieshao", "liexiao":
			return [
				{ "text": "列肖把《燈後微聲》翻開，才看兩行，眉頭便先皺了一下。", "speaker": 2, "portrait": LIE_XIAO_BOOK_PORTRAIT },
...
func _build_book_reading_ink_unfallen(actor_key: String) -> Array:
	match actor_key:
		"lieshao", "liexiao":
			return [
				{ "text": "列肖翻開《未落之墨》，才掃過幾行，眼神便沉了下來。", "speaker": 2, "portrait": LIE_XIAO_BOOK_PORTRAIT },
...
func _build_book_reading_breath_beyond_words(actor_key: String) -> Array:
	match actor_key:
		"lieshao", "liexiao":
			return [
				{ "text": "列肖翻開《字外有息》，才看見那大片空白，便先皺了皺眉。", "speaker": 2, "portrait": LIE_XIAO_BOOK_PORTRAIT },
				...
				{ "text": "(列肖看完最後一句，沉默了一會。)", "speaker": 2, "portrait": LIE_XIAO_BOOK_PORTRAIT },
