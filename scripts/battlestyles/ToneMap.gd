...
var tone_map := {
	...
	"ally_dodge": {
		...
		"lieshao": {
			"default": [
				"列霄懶懶地一側身，像是早就料到對方碰不到自己，那一下便從他身旁空空掠過。",
				"列霄腳下步子看著散漫，偏偏就差那麼一寸，讓來招撲了個空。",
				"眼見攻勢逼近，列霄卻只漫不經心地一晃身，便將那一擊讓得乾乾淨淨。",
				"列霄像是根本沒把這一下放在眼裡，只隨意挪了挪步，對方的勁路便盡數落空。"
			]
		},
		"default": {
			"default": [
	...
	"ally_enemy_defeat": {
		...
		"lieshao": {
			"default": [
				"敵人才剛倒下，列霄便懶懶收回手勢，像是這樣的結果本就不值得大驚小怪。",
				"列霄瞥了倒地的對手一眼，神情裡帶著點淡淡的不耐，像是在說這場鬧劇總算肯收場了。",
				"那一擊落定後，列霄只是把氣息一收，眼神淡淡掠過，彷彿勝負早就寫在前頭。",
				"對手終於支撐不住倒下，列霄神色不動，連唇角都只微微一撇，像是意料之中的收尾。"
			]
		},
		"default": {
			"default": [
	...
	"ally_down_self": {
		...
		"lieshao": {
			"default": [
				"列霄腳下踉蹌了一步，原本還帶著幾分散漫的身形終於再撐不住，重重倒了下去。",
				"列霄本還想用最後一點力氣把身形撐住，卻終究失了支撐，整個人向後倒去。",
				"那一下像是徹底抽空了列霄胸中的氣勁，連他慣有的從容都碎了，身形隨之頹然倒地。",
				"列霄被逼得退了半步又半步，終究還是沒能站住，只得狼狽地倒了下去。"
			]
		},
		"default": {
			"default": [
	...
	"ally_down_reaction": {
		...
		"lieshao": {
			"default": [
				"列霄原本漫不經心的神色忽然冷了下來，連那點懶散都在瞬間收了乾淨。",
				"同伴倒下的一刻，列霄目光微微一沉，原先那副事不關己的模樣終究裂開了一道縫。",
				"列霄看著那道身影倒下，手中動作雖未停，眼神卻比方才更冷、更深了些。",
				"那一瞬間，列霄像是把原本掛在臉上的散漫都收了起來，只剩一股壓得極低的怒意。"
			]
		},
		"default": {
			"default": [
	...
	"innerforce_applied": {
		...
		"破軍": {
			...
			"lieshao": "列霄的目光像在與命運賭一把，破軍心法隨刀勢一同壓下。"
		},
		"靈風": {
	...
	"defend": {
		...
		"lieshao": {
			"default": "列霄左手搭在琴身，右手停在弦上，似退非退，音殺隨時可能再起。"
		}
	},
	...
	"item_use": {
		...
		"mp_heal": {
			...
			"lieshao": "那點藥勁像酒一樣辣，卻把列霄體內散亂的琴音一一收了回來。"
		},
		"buff_speed": {
			...
			# 列霄：半吊兒郎當，但其實節奏算得很準
			"lieshao": "列霄懶懶一甩手，把藥粉灑進風裡，不特意瞄準，卻讓風自己去挑該落在誰腳邊。"
		},
		# 出招的人視角：降低對方速度
		"debuff_speed": {
			...
			"shumian": "書眠指尖一彈，細碎機括像散墨般飛出，把對方的腳步牢牢牽住。"
				...
				"haste_talisman": {
						...
						# 列霄：用琴當發射器，把「速度」丟進樂句裡
						"lieshao": "列霄將那張靈符按壓在琴弦上，指尖一撥，符紋隨弦音震盪飛出，腳步也隨著音律輕了起來。"
				},
				"fire_talisman_ally": {
...
var enemy_dialogue := {
	"opening": {
		...
		"江湖人士": ["請了。", "既已拔兵刃，那便手底下見真章。", "出手之前，先報上名來。"],
		"朝廷": ["奉命行事，退開。", "抗命者，按律不赦。", "朝命在前，莫怪刀下無情。"],
		"default": ["殺機驟起，敵影已逼上前。"],
	},
	"skill": {
		"野獸": ["吼啊——！", "嗷——！", "砰！（暴衝）"],
		"飛禽": ["咻——！（俯衝）", "嘎——！（掠擊）", "撲啦！（振翅逼近）"],
		"爬蟲": ["嘶啊！", "沙——！（疾竄）", "啪！（甩尾）"],
		"機關": ["咔嗒！", "鎖定。", "發射。"],
		"地痞": ["看老子這一手！", "給我躺下！", "吃我一招！"],
		"江湖人士": ["接招。", "得罪了。", "這一式，請指教。"],
	...
	"dodge": {
		...
		"爬蟲": ["嘶——", "沙！（滑開）", "嘶嘶……"],
		"機關": ["閃避。", "偏移成功。", "校正完成。"],
		"地痞": ["嘖，差遠了。", "就這點本事？", "哈，連邊都沒沾著。"],
		"江湖人士": ["慢了。", "這一下，還差些火候。", "出手雖快，仍少了準頭。"],
		"朝廷": ["徒勞。", "招已看破。", "這等手段，還不夠。"],
		"語魅": ["你碰不到我……", "霧裡的影子，如何抓得住？", "再快，也只是徒勞……"],
		"default": ["這一下，落空了。"],
	},
	"debuff_success": {
		"野獸": ["嗚吼！", "哈呲……！", "呼嚕！（逼近）"],
		...
		"爬蟲": ["嘶嘶……", "沙啦……！", "嘶！（纏逼）"],
		"機關": ["命中。", "壓制完成。", "喀——"],
		"地痞": ["怎麼樣？還撐得住嗎？", "嘿，這下難受了吧？", "看你還怎麼硬撐！"],
		"江湖人士": ["勁路已亂，你還撐得了多久？", "這口氣一散，再難回來。", "你的章法，已經亂了。"],
		"朝廷": ["你已無力再抗。", "章法已亂，還想撐？", "到此為止了。"],
	...
	"debuff_suffer": {
		...
		"爬蟲": ["嘶啊！", "沙啦……", "嘶！（受擾）"],
		"機關": ["異常。", "錯誤狀態。", "運作受阻。"],
		"地痞": ["嘖……這是什麼鬼把戲？", "可惡，身子怎麼不聽使喚了！", "你動了什麼手腳？"],
		"江湖人士": ["勁路……亂了。", "這一下，倒是有些門道。", "氣息被你打岔了……"],
		"朝廷": ["章法受擾……", "可惡，氣勁被截住了。", "這等旁門手段……"],
		"語魅": ["你竟敢……反過來污染我……", "這股雜音……竟侵了進來……", "不該是這樣……不該是這樣……"],
		"default": ["氣息驟亂，身形明顯一滯。"],
	},
	"down": {
		"野獸": ["嗚……", "嗷啊！", "砰……！（伏地）", "嗚咽……！"],
		"飛禽": ["嘎……！", "撲通！（墜地）", "喀噠……！（翅膀拍地）", "戾啊——！（失速墜落）"],
		"爬蟲": ["嘶……", "沙……（蜷縮）", "啪嗒……！（癱伏）"],
		"機關": ["錯誤……", "喀啦……！", "嗡……終止……", "停擺。"],
		"地痞": ["可惡……這筆帳……", "啐……今天算你走運……", "這次……算老子倒楣……"],
		"江湖人士": ["……果然還是技不如人。", "這一敗，我認。", "再也……提不起這口氣了……"],
		"朝廷": ["可笑……我這頂官帽，竟也掉了……", "回去之後……怕是難逃究責……", "朝命未成……我竟先倒在這裡……", "官身至此……竟成笑柄……"],
...
var yumei_dialogue := {
	"opening": {
		"磐石": ["何必要如此拼命？", "當顆石頭，不好嗎？", "別再動了……"],
		"滯水": ["留下來吧……", "越掙扎，只會沉得越快……", "任何事物，終究都會消逝……"],
	},
	"skill": {
	...
	"debuff_success": {
		...
		"滯水": ["看吧，連腳步都慢下來了。", "你已經開始下沉了。", "放手之後，一切都會安靜。"],
	},
	"debuff_suffer": {
		"磐石": ["凝住的……怎會被撼動……", "不，不該再動了……", "裂了……那股寂靜，裂了……"],
		"滯水": ["水流……竟亂了……", "我明明已將你拖住……", "這股逆流……好吵……"],
	},
	"down": {
		"磐石": ["原來……石頭也會裂開……", "你們……為何不肯停下……", "凝住的東西……竟也會崩散……"],
		"滯水": ["水……竟也有盡頭……", "原來……還有人能逆流而上……", "這股水聲……終於……也要停了……"],
	},
}
...
var hit_reactions := {
	"ally": {
		...
		"lieshao": {
			"normal": [
				"列霄眉梢一跳，琴弦微顫，胸口一悶卻只冷笑出聲。",
				"力道毫不遮掩地透過琴身傳入臂骨，他不只挨了一擊，也被撞亂一弦音。"
			],
			"weak": [
				"那股力道順勢擊在他尚未復平的舊傷上，列霄一時連琴音都壓低了半分。",
				"勁氣沿著經絡逆流而上，他指尖一抖，差點掐錯弦。"
			],
			"def_normal": [
				"列霄抬手護胸，餘勁透過臂骨傳來，但微不足道的痛意絲毫不影響他的笑意半分。",
				"琴身橫在身前替列霄擋下大半衝擊，只剩微不足道的餘波稍微擾了琴音幾許，律尚在。"
			],
			"def_weak": [
				"明知不利仍強撐著將餘勁導入琴身，琴音一顫，彷彿替他挨了這一記。",
				"那一下幾乎要將他的護身音勁打散，列霄只用更懶散的笑意壓住臉色的發白。"
			]
		},
